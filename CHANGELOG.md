...
### Added
- **Escalas › Portaria: escala mensal automática** — replaces the manual door rotation. Admins/portaria managers add the workers (searching the directory or typing a name), tick the service days each one can cover (Segunda, Terça, Quarta, Quinta, Sábado, EBD, Domingo) and save. "Gerar escala" then builds any month (current or another): each service gets the chosen number of workers (1-4), only from people who marked that day. The generator fills the hardest services first and avoids repeating a person in the same week or on the same weekday two weeks running, while keeping the load proportional to each person's availability — someone who only marked Wednesday is put on Wednesdays (alternating when others are also free). Services nobody can cover are listed as warnings. The month shows as a calendar; each service can be adjusted by hand (✕ to remove, "+ obreiro" to add), and "Gerar novamente" asks before replacing it. "Publicar" opens the read-only month calendar at `/portaria/YYYY-MM` (alias `/door-schedule/…`; outside `PublicLayout`, list layout on phones), with prev/next month navigation and PT/EN. Generator and calendar helpers in `src/lib/doorSchedule.js` (tests in `src/test/doorSchedule.test.js`).
- **Culto Profético › Lista de Louvores (MVP)** — public, PIN-free page `/culto-profetico` (alias `/prophetic-service`; outside `PublicLayout`, so it stays open in maintenance mode) where whoever is typing picks the date and service (EBD / Noite / Outro), fills group, praise leader and "Mensagem" (preacher) as free text, and builds the list of praise songs with one search box for number or title (accent-insensitive; `cl 1` finds `CL-001`). Songs that don't exist yet can be added on the spot (category, optional code, title, theme for Coletânea/CIA); they enter as *pending*, with a warning when a similar title or the same code already exists. The typist's name is required to save and is remembered on the device. After saving, songs can be added, removed and reordered during the service (each change is saved at once). Anyone with the link can keep editing the list (so the task can pass to someone else during the service) until it is published or locks itself at 06:00 the next morning; admins can always edit, lock or unlock. Search results show "Cantado há N dias" for songs sung in the last 90 days; the **Histórico** tab shows every list of any date (lists are kept forever) and the latest dates. Bilingual (PT/EN, `praise*` keys).
- **Lista de Louvores publicada para o Grupo de Louvor** — when the service ends, "Finalizar e publicar" closes the list for editing, copies the link and opens the read-only page `/culto-profetico/<id>` (alias `/prophetic-service/<id>`): the portal's look with the layout of the paper list — "LOUVORES" and "DOMINGO - 02/08/2026", then N° | Nome do Hino (number only — `CL-004` shows as 4 —, or `AVL`/`CIA`/`ENG` for songs without one), plus service, group, leader and preacher in small type. A list that is not published shows "ainda não foi publicada". Admins see a "Publicada" badge and can copy the link or "Reabrir" (unlock and unpublish) from Escalas › Lista de Louvores.
- **Igreja na lista** — each list has a required "Igreja" (Newark, New York, Philadelphia, Toms River first, then every other church of the directory; the last choice is remembered on the device). The published page shows it under "IGREJA CRISTÃ MARANATA" (city only, e.g. NEWARK).
- **Admin manages every list** — Escalas › Lista de Louvores › Listas shows all lists (not just the latest) and lets admins and the Grupo de Louvor leader open any of them in the same editor as the public page — even a published/locked one or one created on another device — to change church, group, leader, preacher, add/remove/reorder songs, "Nova lista" to create one for any date and service, and delete. Songs an admin creates are approved right away.
- **Escalas › Lista de Louvores (admin)** — replaces the old Músicas tab: approve/edit/delete pending songs, browse and edit the catalogue (add approved songs directly), see every list with its songs, lock/unlock or delete lists, copy the public link. Approving, editing and deleting is limited to admins and the Grupo de Louvor team leader; everyone else sees it read-only.
- Song codes by category: `CL-001` (Coletânea), `CIA-012`, `ENG-031` (English), `AVL-…` (avulso; usually no code). Catalogue starts empty — the old numbered list will be imported later.
...
- Prayer board: slots with several people are laid out as a header (time + "n/cap") with one row per person (full name, church label, "(você)", ✕) that wraps instead of squeezing the name to nothing; same fix in the admin grid.
- **Several people per prayer slot, configurable per list** — new field "Pessoas por horário" (1-20, default 1 = old behaviour) when creating/editing a list. A slot with someone but room left stays selectable and shows its capacity ("1/3"); on confirm the public board warns "Esse período já tem alguém orando…" and lets the person proceed; a full slot locks. The board explains the capacity and the goal (someone praying in every slot) when capacity > 1. Progress counts slots with someone praying. Admin grid lists every person in a slot (each removable); WhatsApp copy joins names with " / "; landing cards show the capacity; both spreadsheet/previous-period imports respect it.
- **Spreadsheet import for prayer lists** — Escalas › Oração 24h › "Importar planilha": upload the filled-in template, review a table of every row (invalid times, church outside the list's scope, slots already taken, repeated slots, several names in one cell) and import only the valid ones. Template (`public/modelos/modelo-importacao-oracao.xlsx`) has the 96 time slots pre-filled, a church dropdown and instructions. Optional "church for rows without one". Adds the `read-excel-file` dependency (loaded only when a file is chosen).
- Landing page legend under the cards explaining the status badges (Em andamento / Em breve / Encerrada).
...
### Database
- `migrations/027_worship_list_church.sql` — adds `worship_lists.church` (text, max 120, NULL for older lists). **Apply before deploying the code that uses it**: saving a list writes this column (`supabase db query --linked -f migrations/027_worship_list_church.sql`).
- `migrations/026_worship_list_published.sql` — adds `worship_lists.published_at` (timestamptz, NULL = not published). **Apply before deploying the code that uses it** (`supabase db query --linked -f migrations/026_worship_list_published.sql`).
- `migrations/025_praise_lists.sql` — creates `praise_songs` (code unique case-insensitively when present, category `coletanea|cia|avulso|english`, optional theme, status `active|pending`, `legacy_number` for the later import), `worship_lists` (date, `service_type`, group, leader, preacher, `entered_by`, `locked`) and `worship_list_items` (unique per list + song, cascade on list delete), and the `praise_song_last_sung` view (last date and times sung per song), all with RLS disabled and grants. **Apply before deploying the code that uses it** (`supabase db query --linked -f migrations/025_praise_lists.sql`) and verify `select relrowsecurity from pg_class where relname in ('praise_songs','worship_lists','worship_list_items');` returns `false` for all three.
...
- `migrations/024_prayer_slot_capacity.sql` — `prayer_periods.slot_capacity`, drops `UNIQUE (period_id, slot_index)` from `schedule_oracao`, unique index per person per slot, and a capacity-enforcing `BEFORE INSERT` trigger (validated locally against a Postgres engine). **Must be applied before the code that uses it is deployed** (`supabase db query --linked -f migrations/024_prayer_slot_capacity.sql`).
...
### Changed
- **UX polish pass (Phase 1 of `DESIGN.md`'s plan)** — buttons now press with a `scale(.97)` on `:active` app-wide (they previously had no click feedback at all); modals fade+scale in on open (`@starting-style`, CSS-only, no JS); the toast now transitions out instead of vanishing when its timer fires; the Portaria calendar's "no worker" label switched from a hard `#b91c1c` red to the app's existing `--warn` amber, so a day nobody's covered yet reads as "needs attention" rather than "broken."
- "Músicas" renamed to "Lista de Louvores" (tab, headings and empty states); the old `SetlistTab` and `/songs/:date` view are no longer linked from the Escalas tab but were left in place.
- Page title and link preview (WhatsApp, etc.) now say "MCC Newark Hub" with a short description, instead of the leftover template title `events-app` (`index.html`: `<title>`, description, Open Graph tags).
...
- Escalas › Portaria: worker names and section titles were unreadable in dark mode (`DoorTab.jsx`/`DoorCalendar.jsx` left `color` unset on several elements, so they inherited `<body>`'s light-mode color instead of the theme's — dark mode is applied on a div below `<body>`, so anything that doesn't explicitly redeclare `color: var(--text)` keeps the light value). Also replaced its local, non-dismissing success/error banner with the shared `notify()` toast (matching `PraiseListsTab.jsx`'s convention) so messages read the same as the rest of the app and disappear on their own.
